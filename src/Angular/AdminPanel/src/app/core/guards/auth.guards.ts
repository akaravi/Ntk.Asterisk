...
/** Settings / ACL admin pages — require admin when gate is active. */
export const adminRoleGuard: CanActivateFn = () => {
  ...
  return auth.refreshStatus().pipe(
    map((s) => {
      if (!s?.gateActive) return true;
      if (s.isAuthenticated && s.isAdmin) return true;
      if (!s?.isAuthenticated) return router.createUrlTree(['/login']);
      return router.createUrlTree(['/queues']);
    }),
    catchError(() => of(true)),
  );
};
