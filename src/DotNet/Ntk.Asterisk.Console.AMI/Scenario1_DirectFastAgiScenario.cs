...
public class Scenario1_DirectFastAgiScenario : SmartRouteScenarioBase
{
        ...
        try
        {
            ...
            for (var i = 0; i < steps.Count; i++)
            {
                ...
                if (!string.IsNullOrWhiteSpace(ext))
                {
                    ...
                    var dialStatus = ExecuteDialAndGetStatus(channel, extDial);
                    if (string.Equals(dialStatus, "HANGUP", StringComparison.OrdinalIgnoreCase))
                        return;
                    ...
                    if (!string.IsNullOrWhiteSpace(mobile))
                    {
                        ...
                        var mobStatus = ExecuteDialAndGetStatus(channel, mobDial);
                        if (string.Equals(mobStatus, "HANGUP", StringComparison.OrdinalIgnoreCase))
                            return;
    ...
    private static string ExecuteDialAndGetStatus(AGIChannel channel, string dialString)
    {
        try
        {
            channel.SendCommand(new ExecCommand("Dial", dialString));
            var reply = channel.SendCommand(new GetVariableCommand("DIALSTATUS"));
            return reply?.Extra ?? string.Empty;
        }
